...
import { setMusicVolume, getMusicVolume, switchGenre, getCurrentGenre, getCurrentTrackIndex, setYTPlayer, destroyYTPlayer, nextTrack, isStarted, MUSIC_GENRES } from '../../utils/audioManager';
...
const NavigationUI = () => {
    ...
    const [activeGenre, setActiveGenre] = useState(() => getCurrentGenre());
    const [ytTrackIdx, setYtTrackIdx] = useState(() => getCurrentTrackIndex());
    ...
    useEffect(() => {
        ...
        setActiveGenre(getCurrentGenre());
        ...
        const handleTrackChange = (e) => {
            setYtTrackIdx(e.detail);
        };
        window.addEventListener('musicVolumeChanged', handleMusicVolumeChange);
        window.addEventListener('genreChanged', handleGenreChange);
        ...
        return () => {
            ...
            window.removeEventListener('genreChanged', handleGenreChange);
    ...
    return (
        <div className="navigation-ui">
            ...
            {/* Hidden YouTube Player for genre music */}
            {(() => {
                const genre = MUSIC_GENRES.find(g => g.id === activeGenre);
                const shouldPlayYT = genre && !genre.useLocalTrack && genre.id !== 'off' && genre.ytIds && genre.ytIds.length > 0;

                if (!shouldPlayYT) return null;

                const videoId = genre.ytIds[ytTrackIdx % genre.ytIds.length];
                return (
                    <div key={`yt-container-${activeGenre}-${ytTrackIdx}`} style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden', pointerEvents: 'none' }}>
                        <YouTube
                            videoId={videoId}
                            opts={{
                                height: '1',
                            ...
                            onReady={(e) => {
                                // Stop any previous player first
                                if (ytPlayerInstanceRef.current && ytPlayerInstanceRef.current !== e.target) {
                                    try { ytPlayerInstanceRef.current.stopVideo(); } catch (err) {}
                                }
                                ytPlayerInstanceRef.current = e.target;
                                setYTPlayer(e.target);
                                e.target.setVolume(bgmVol * 100);
                                e.target.playVideo();
                            }}
                            onEnd={() => {
                                nextTrack();
                ...
                <div className={`audio-panel ${isAudioMenuOpen ? 'open' : ''}`} inert={!isAudioMenuOpen ? true : undefined}>
                    <div className="audio-card">
                        ...
                        <div className="genre-selector">
                            <div className="genre-selector-label">Choose Your Vibe</div>
                                ...
                                {MUSIC_GENRES.map((genre) => (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            switchGenre(genre.id);
                                            setActiveGenre(genre.id);
                                        }}
                                        style={{
                                            borderColor: activeGenre === genre.id ? genre.color : 'transparent'
                                        }}
                                    >
                                        <span className="genre-dot" style={{ background: genre.color }} />
                                        <div className="genre-info">
                                            <span className="genre-name">{genre.name}</span>
                                            <span className="genre-desc">{genre.description}</span>
                                        </div>
                                        {activeGenre === genre.id && genre.id !== 'off' && (
                                            <span className="genre-playing">▶</span>
                                        )}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="audio-now-playing">
                            Now Playing: <strong>{MUSIC_GENRES.find(g => g.id === activeGenre)?.name || 'Ambient'}</strong>
                        </div>
                    </div>
                </div>
            )}
